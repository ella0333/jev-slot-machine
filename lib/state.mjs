 ...
 */
export function freshState(now = Date.now(), { run = 1, balance = 1200, lifetime } = {}) {
  return {
    version: 1,
    run,
    startedAt: now,
    ...
    recentPurchases: [],  // bought on this trip away from the machine, cleared on return
    eventSeq: 0,
  };
}
...
export function applyPurchase(state, item) {
  ...
  // A trip ends the moment Jev goes back to the machine, so this list is short
  // by nature. The cap is only a guard against a very long trip: it sits above
  // the size of the menu, so Jev can never be shown an item Jev bought earlier
  // on this trip and have forgotten it.
  if (state.recentPurchases.length > 12) state.recentPurchases.shift();
}
...
export function enterMode(state, mode, at) {
  ...
  if (mode === MODES.GAMBLING) {
    ...
    state.lossStreak = 0;
    // The purchase list is only there to stop Jev buying the same thing twice
    // on one trip away from the machine, so it ends when the trip does.
