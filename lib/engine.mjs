...
/* Pace score maps onto the gap between spins. A real player at a slot machine
 * is somewhere between four and twenty five seconds, and the slow end is
 * somebody who has stopped enjoying it. */
const FAST_MS = 4200;
const SLOW_MS = 25000;
...
async function gamblingTurn(state, { config, dry }) {
  ...
  const canSpend = config.allowPurchases && canShop(state.balance);
  ...
  const action = canSpend
    ? decide(a.action, { floor: 0.5, fallback: 'spin_again' })
...
async function shoppingTurn(state, { config, dry }) {
  const menu = buildMenu(state.balance);
  ...
  const next = decide(a.after_purchase, { floor: 0.45, fallback: 'back_to_slots' });
  ...
  if (next.value === 'buy_something_else' && canShop(state.balance)) {
    // Stays in SHOPPING for another turn.
  } else {
    enterMode(state, MODES.GAMBLING, state.plannedUntil);
    events.push(makeEvent(state, state.plannedUntil, 'mode', { mode: MODES.GAMBLING, reason: 'back to the slot machine' }));
  }
