...
/**
 * Read a Choice, falling back when there is no usable answer.
 *
 * `choice` is documented as the highest probability option, so it is already
 * the decision. Confidence is a reported measure of how certain Jev was, not a
 * veto: a floor above zero here silently replaces Jev's answer with our own,
 * which is why every caller passes zero and records the confidence alongside
 * the outcome instead. The fallback exists for a missing or malformed answer,
 * nothing else.
 */
