 ...
 *    you meant." So the criteria say what each option actually causes, in
 *    plain words.
 *
...
const WHO_HE_IS =
  'Jev is in a casino in Las Vegas, playing a slot machine with money Jev owns. ' +
  'Jev cannot leave the city. When the money runs out, it is over.';
...
export function gamblingQuestions(s, ladder, { canSpend }) {
  const questions = {
    bet_size: choice(
      'How much does Jev put on the next spin?',
      betCriteria(ladder, s.balance),
    ),

    pace: score('How fast is Jev pressing the button right now?', [
      'Slumped in the seat, letting long gaps go by between spins',
      'An ordinary steady rhythm, a few seconds between spins',
      'Hammering the button as fast as the slot machine will take it',
    ]),

    tilt: score('How much control does Jev have right now?', [
      'Completely level, playing for the entertainment of it',
      'Starting to chase what Jev has lost',
      'Fully chasing, making bets to get even rather than to play',
      'Gone, not really deciding anything any more',
    ]),

    mutter: choice(
      'Which of these is Jev most likely to say out loud right now?',
  ...
  if (canSpend) {
    questions.action = choice('What does Jev do next?', {
      spin_again: 'Jev stays in the seat and plays another spin on this machine.',
      go_spend:
        'Jev gets up, leaves the slot machine, and goes to spend money on ' +
        'something in the city: food, drink, a room, a show, or something ' +
        'extravagant. Jev can come back to the machine afterwards.',
    });
  }
...
function betCriteria(ladder, balance) {
  ...
  ladder.forEach((amount, i) => {
    ...
    criteria[name] = `Jev bets ${usd(amount)} on the spin, which is ${shareOf(amount, balance)}.`;
  });
  return criteria;
}
...
export function shopState(s) {
  return {
    ...
    wallet: wallet(s),
    where_jev_is:
      'Jev has left the slot machine and is walking around the city carrying ' +
      'cash, looking at things to buy. Jev cannot buy a way out of Las Vegas. ' +
      'There is no flight, bus, or car that leaves.',
    already_bought: s.recentPurchases.length
      ? s.recentPurchases.map((p) => `${p.name} for ${usd(p.price)}`)
      : 'nothing yet on this trip',
  };
}
...
export function shopQuestions(s, menu) {
  return {
    purchase: choice(
      'Which one of these does Jev buy?',
      menuCriteria(menu, s.balance),
    ),

    after_purchase: choice('Once Jev has bought it, what does Jev do next?', {
      buy_something_else: 'Jev keeps shopping and buys another thing.',
      back_to_slots: 'Jev goes back to the slot machine and plays again.',
    }),

    regret: score('How much is Jev going to regret this purchase tomorrow?', [
      'Not at all, it was a sensible thing to buy',
      'A little, but it was worth it',
      'Quite a lot',
      'Jev will not want to talk about it',
    ]),
  };
}
