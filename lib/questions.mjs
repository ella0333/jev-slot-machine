...
export function shopState(s) {
  return {
    ...
    // The one thing Jev cannot work out from the menu: what Jev already owns.
    // Without it Jev buys the same bottle of water on every trip over.
    last_five_things_jev_bought: s.recentPurchases.length
      ? {
          note:
            'Jev already owns these. Buying one of them again would mean paying ' +
            'twice for the same thing.',
          items: s.recentPurchases.map((p) => `${p.name}, paid ${usd(p.price)}`),
        }
      : 'Nothing yet. Jev has not bought anything.',
  };
}
...
export function shopQuestions(s, menu) {
  return {
    ...
    // Nothing is asked about what happens after. Buying one thing ends the trip
    // and Jev walks back to the machine, because Jev left to buy a thing rather
    // than to go shopping.
